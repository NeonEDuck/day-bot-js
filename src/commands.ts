...
import { EventsWithListener, isArrayOfEventsWithListenerType } from './type/events.ts'

const commands: RESTPutAPIApplicationCommandsJSONBody = []
const collection = {
    commands: new Collection<string, CommandType>(),
    events: [] as ReturnType<typeof EventsWithListener>[],
}
...
for (const folder of commandFolders) {
    ...
    for (const file of commandFiles) {
        ...
        if (command && isCommandType(command)) {
            ...
            console.log(`[WARNING] The default export of "${filePath}" does not match type with "CommandType".`)
            continue
        }
