...
export const combination = <T, V, U>(arr1: T[], arr2: V[], callbackfn: (x: T, y: V)=>U): U[] => arr1.flatMap(d => arr2.map(v => callbackfn(d, v)))
export const zip = (...rows: any[][]) => rows[0].map((_, c)=>rows.map(row=>row[c]))
export const groupBy = <T, V extends object|{}>(arr: T[], fn: (element:T)=>V) => {
